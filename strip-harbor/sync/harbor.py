...
harbor_projects_path = '/projects/106'
harbor_project_path = '/project/106'
...
def create_harbor_user_from_customer(customer_email,strip_id):
    if not customer_email:
        raise ValueError("Couldn't create a harbor user for customer %s - the record doesn't have the email set" % (strip_id))
    return create_robot_account_for_project(customer_email_to_harbor_username(customer_email))
